...
{
	auto widget = Gtk::make_managed<gtk::HVNumWidget>(Gtk::Orientation::HORIZONTAL,adj_,0, units::percent);
	append(*widget);
	widget->set_vexpand();
...
{
	auto widget = Gtk::make_managed<gtk::HVNumWidget>(Gtk::Orientation::HORIZONTAL,adj_,0, units::hz);
	append(*widget);
	widget->set_vexpand();

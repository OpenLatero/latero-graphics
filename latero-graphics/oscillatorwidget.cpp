...
{
	auto widget = Gtk::make_managed<gtk::NumWidget>(Gtk::Orientation::HORIZONTAL,adj_,0, gtk::name_none,units::percent);
	append(*widget);
	widget->set_vexpand();
...
{
	auto widget = Gtk::make_managed<gtk::NumWidget>(Gtk::Orientation::HORIZONTAL,adj_,0, gtk::name_none, units::hz);
	append(*widget);
	widget->set_vexpand();
